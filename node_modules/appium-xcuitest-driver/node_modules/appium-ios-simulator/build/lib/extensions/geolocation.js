...
async function setLocationWithAppleScript(sim, latitude, longitude, menu = 'Debug') {
    ...
    const { stdout, stderr } = await (0, teen_process_1.exec)('osascript', ['-e', DECIMAL_SEPARATOR_SCRIPT]);
    const decimalSeparator = lodash_1.default.trim(stdout || stderr);
  ...
  `);
    logger_1.default.debug(`Geolocation parameters dialog accepted: ${output}`);
    if (lodash_1.default.trim(output) !== 'true') {
